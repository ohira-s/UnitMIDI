...
class message_definitions():
  def __init__(self):
    ...
    self.MSGID_DEVICE_TOUCH_DELEGATE_EVENT_TOUCH = 10101
    self.MSGID_DEVICE_TOUCH_DELEGATE_EVENT_RELEASE = 10102
    self.MSGID_DEVICE_TOUCH_DELEGATE_EVENT_START_PRESSING = 10103
    self.MSGID_DEVICE_TOUCH_DELEGATE_EVENT_DURING_PRESSING = 10104
    self.MSGID_DEVICE_TOUCH_DELEGATE_EVENT_FINISH_PRESSING = 10105
    self.MSGID_DEVICE_TOUCH_DELEGATE_EVENT_START_DRAGGING = 10106
    self.MSGID_DEVICE_TOUCH_DELEGATE_EVENT_DURING_DRAGGING = 10107
    self.MSGID_DEVICE_TOUCH_DELEGATE_EVENT_FINISH_DRAGGING = 10108
...
#          self.message_center.phone_message(self, self.message_center.MSGID_MIDI_ALL_NOTES_OFF, {'channel': ch})
      ...
      # Change screen mode
        ...
        if enc_button:
          self.message_center.phone_message(self, self.message_center.MSGID_APPLICATION_PAD_CONTROLLER)
...
#      self.midi_in_settings.append({'program':0, 'gmbank':0, 'reverb':[0,0,0], 'chorus':[0,0,0,0], 'vibrate':[0,0,0]})
  ...
  # Add a pad
  def add_pad(self, shape):
    self.pad_shapes.append(shape)

  # Find a pad at coordinates
...
##########################
# Touch Pad Device Class
##########################
class device_touch_pad_class(message_center_class):
  # Constructor
  def __init__(self, device_manager, message_center = None):
    self.touch = False
    self.pressing = False
    ...
    self.touch_x = None
    self.touch_y = None
    self.drag_points = []         # Array of tuples of dragging point [(1st touch x,1st touch y), (2nd...), ...]
    self.delegate_event_touch = None
    self.delegate_event_release = None
    self.delegate_event_start_pressing = None
    self.delegate_event_during_pressing = None
    self.delegate_event_finish_pressing = None
    self.delegate_event_start_dragging = None
    self.delegate_event_during_dragging = None
    self.delegate_event_finish_dragging = None

    device_manager.add_device(self)
    if not message_center is None:
      self.message_center = message_center
      self.message_center.add_contributor(self)
      self.message_center.add_subscriber(self, self.message_center.MSGID_DEVICE_TOUCH_DELEGATE_EVENT_TOUCH, self.func_DEVICE_TOUCH_DELEGATE_EVENT_TOUCH)
      self.message_center.add_subscriber(self, self.message_center.MSGID_DEVICE_TOUCH_DELEGATE_EVENT_RELEASE, self.func_DEVICE_TOUCH_DELEGATE_EVENT_RELEASE)
      self.message_center.add_subscriber(self, self.message_center.MSGID_DEVICE_TOUCH_DELEGATE_EVENT_START_PRESSING, self.func_DEVICE_TOUCH_DELEGATE_EVENT_START_PRESSING)
      self.message_center.add_subscriber(self, self.message_center.MSGID_DEVICE_TOUCH_DELEGATE_EVENT_DURING_PRESSING, self.func_DEVICE_TOUCH_DELEGATE_EVENT_DURING_PRESSING)
      self.message_center.add_subscriber(self, self.message_center.MSGID_DEVICE_TOUCH_DELEGATE_EVENT_FINISH_PRESSING, self.func_DEVICE_TOUCH_DELEGATE_EVENT_FINISH_PRESSING)
      self.message_center.add_subscriber(self, self.message_center.MSGID_DEVICE_TOUCH_DELEGATE_EVENT_START_DRAGGING, self.func_DEVICE_TOUCH_DELEGATE_EVENT_START_DRAGGING)
      self.message_center.add_subscriber(self, self.message_center.MSGID_DEVICE_TOUCH_DELEGATE_EVENT_DURING_DRAGGING, self.func_DEVICE_TOUCH_DELEGATE_EVENT_DURING_DRAGGING)
      self.message_center.add_subscriber(self, self.message_center.MSGID_DEVICE_TOUCH_DELEGATE_EVENT_FINISH_DRAGGING, self.func_DEVICE_TOUCH_DELEGATE_EVENT_FINISH_DRAGGING)
    else:
      self.message_center = self

  def func_DEVICE_TOUCH_DELEGATE_EVENT_TOUCH(self, message_data = None):
    if message_data is None:
      self.delegate_event_touch = None
    else:
      self.delegate_event_touch = message_data['delegate']    

  def func_DEVICE_TOUCH_DELEGATE_EVENT_RELEASE(self, message_data = None):
    if message_data is None:
      self.delegate_event_release = None
    else:
      self.delegate_event_release = message_data['delegate']    

  def func_DEVICE_TOUCH_DELEGATE_EVENT_START_PRESSING(self, message_data = None):
    if message_data is None:
      self.delegate_event_start_pressing = None
    else:
      self.delegate_event_start_pressing = message_data['delegate']    
  ...
  def func_DEVICE_TOUCH_DELEGATE_EVENT_FINISH_PRESSING(self, message_data = None):
    if message_data is None:
      ...
      self.delegate_event_finish_pressing = message_data['delegate']    

  def func_DEVICE_TOUCH_DELEGATE_EVENT_START_DRAGGING(self, message_data = None):
    if message_data is None:
      self.delegate_event_start_dragging = None
    else:
      self.delegate_event_start_dragging = message_data['delegate']    

  def func_DEVICE_TOUCH_DELEGATE_EVENT_DURING_DRAGGING(self, message_data = None):
    if message_data is None:
      self.delegate_delegate_event_during_dragging = None
    else:
      self.delegate_delegate_event_during_dragging = message_data['delegate']    

  def func_DEVICE_TOUCH_DELEGATE_EVENT_FINISH_DRAGGING(self, message_data = None):
    if message_data is None:
      self.delegate_event_finish_dragging = None
    else:
      self.delegate_event_finish_dragging = message_data['delegate']    

  def event_touch(self, touch_x, touch_y):
    if not self.delegate_event_touch is None:
      self.delegate_event_touch(touch_x, touch_y)

  def event_release(self, touch_x, touch_y, touch_ticks):
    if not self.delegate_event_release is None:
      self.delegate_event_release(touch_x, touch_y, touch_ticks)

  def event_start_pressing(self, touch_x, touch_y, touch_ticks):
    if not self.delegate_event_start_pressing is None:
      self.delegate_event_start_pressing(touch_x, touch_y, touch_ticks)
  ...
  def event_start_dragging(self, drag_points, touch_ticks):
    if not self.delegate_event_start_dragging is None:
      self.delegate_event_start_dragging(drag_points, touch_ticks)

  def event_during_dragging(self, drag_points, touch_ticks):
    if not self.delegate_event_during_dragging is None:
      self.delegate_event_during_dragging(drag_points, touch_ticks)

  def event_finish_dragging(self, drag_points, touch_ticks):
    if not self.delegate_event_finish_dragging is None:
      self.delegate_event_finish_dragging(drag_points, touch_ticks)

  # Device controller
  #   Read input informatiom
  def controller(self):
    # Get touch info and CPU ticks
    touch_count = M5.Touch.getCount()
    t_ticks = time.ticks_us()
    ...
    if touch_count > 0:
      ...
      if self.touch == False:
        ...
        self.touch_x = tch_x
        self.touch_y = tch_y
        self.drag_points = [(self.touch_x, self.touch_y)]
        self.event_touch(self.touch_x, self.touch_y)
        self.touch_ticks = t_ticks

      # Keep touching
      else:
        # Ticks count from touched
        d_touch_ticks = time.ticks_diff(t_ticks, self.touch_ticks)

        # Pressing the same coordinates
        if tch_x == self.touch_x and tch_y == self.touch_y:
          # First sense pressing
          if self.pressing == False:
            self.pressing = True
            self.press_ticks = t_ticks
            self.event_start_pressing(self.touch_x, self.touch_y, d_touch_ticks)
        ...
        # Dragging
          ...
          # Add the drraing point
          self.touch_x = tch_x
          self.touch_y = tch_y
          self.drag_points.append((self.touch_x, self.touch_y))
          ...
          if self.dragging == False:
            self.dragging = True
            self.event_start_dragging(self.drag_points, d_touch_ticks)
          
          # Keep dragging
          else:
            self.event_during_dragging(self.drag_points, d_touch_ticks)
    ...
    # Not touched
      ...
      if self.dragging:
        self.dragging = False
        self.event_finish_dragging(self.drag_points, d_touch_ticks)
        self.drag_points = []

      # Finish pressing by releasing
      if self.pressing:
        self.pressing = False
      ...
      if self.touch:
        self.touch = False
        self.event_release(self.touch_x, self.touch_y, d_touch_ticks)
        self.touch_x = None
        self.touch_y = None

################# End of Touch Pad Device Class Definition #################


class view_pad_controller_class(view_m5stack_core2):
  # Constructor
  def __init__(self, pad_controller_obj, message_center = None):
    super().__init__()

    # Refer sequencer object data to draw GUI
    self.data_obj = pad_controller_obj
...
if __name__ == '__main__':
  try:
    ...
    device_touch_pad = device_touch_pad_class(device_manager, message_center)
    ...
    pad_controller_obj = pad_controller_class(midi_obj, sdcard_obj)
